    ...
    private void deleteCookies(HttpServletRequest request,
        ...
        if (cookies != null) {
            for (Cookie cookie : cookies) {
                // Tránh đụng JSESSIONID nếu bạn muốn giữ nguyên session; 
                // nếu muốn logout hẳn thì có thể cho qua luôn.
                if (!"JSESSIONID".equals(cookie.getName())) {
                    cookie.setValue("");
                    cookie.setPath("/");     // PHẢI khớp path khi tạo
        ...
        // Dọn session user (tùy nhu cầu: remove 1 attr hay invalidate toàn bộ)
        HttpSession session = request.getSession(false);
        if (session != null) {
            session.removeAttribute("user");
            // hoặc: session.invalidate();
        }

        // Quan trọng: REDIRECT để trình duyệt gửi request mới (cookie đã mất)
